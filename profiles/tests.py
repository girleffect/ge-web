...
from .forms import (
    ForgotPasswordForm, RegistrationForm, ProfilePasswordChangeForm, DoneForm, EditProfileForm)
from .models import (
    SecurityQuestion, SecurityQuestionAnswer,
    GEUser, SecurityQuestionIndexPage, GEUserSettings
)
...
class ProfilesTestCaseMixin(object):
    ...
    def setup_cms(self):
        ...
        # Create index page
        self.index = SecurityQuestionIndexPage(title="Security Questions", slug="security-questions")
...
class RegistrationDone(TestCase, ProfilesTestCaseMixin):
    ...
    def test_gender_on_done(self):
        ...
        response = self.client.post("/profiles/register/done/", {
            'gender': 'male',
        })
...
class MyProfileEditTest(TestCase, ProfilesTestCaseMixin):
    ...
    def test_update_dob(self):
        ...
        self.assertRedirects(
            response, "/profiles/view/myprofile/")
        self.assertEqual(GEUser.objects.get(user=self.user).date_of_birth,
                         date(2000, 1, 1))
    ...
    def test_update_gender(self):
        ...
        self.assertRedirects(
            response, "/profiles/view/myprofile/")
        self.assertEqual(GEUser.objects.get(user=self.user).gender,
                         'male')
    ...
    def test_update_location(self):
        ...
        self.assertRedirects(
            response, "/profiles/view/myprofile/")
        self.assertEqual(GEUser.objects.get(user=self.user).location,
                         'mlazi')
