    ...
    def handle(self, *args, **options):
        with open('sections.json', 'r', encoding='utf-8') as f:
            ...
            for section in sections.keys():
                ...
                try:
                    new_section = SectionPage.objects.get(locale=locale, title=sections[section]["title"])
                except SectionPage.DoesNotExist:
                    new_section = SectionPage(locale=locale, title=sections[section]["title"], slug=sections[section]["slug"])
                    if "translation_pks" in sections[section].keys():
                        ...
                        try:
                            trans_section = SectionPage.objects.get(slug=trans_slug, locale__language_code=trans_locale)
                            new_section.translation_key = trans_section.translation_key
                        except:
                            pass
                    if "section_parent_pk" in sections[section].keys():
                        parent_slug = sections[sections[section]["section_parent_pk"]]["slug"]
                        parent_locale = sections[sections[section]["section_parent_pk"]]["locale"]
                        parent = SectionPage.objects.get(slug=parent_slug, locale__language_code=parent_locale)
                    else:
                        parent = HomePage.objects.get(locale=locale).get_children().filter(title="Sections").first()
